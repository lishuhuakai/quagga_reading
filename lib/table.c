...
route_node_set (struct route_table *table, const struct prefix *prefix)
{
    ...
    node = route_node_new (table); /* 创建一个路由节点 */
...
route_node_get (struct route_table *const table, const struct prefix *p)
{
    ...
    u_char prefixlen = p->prefixlen; /* 前缀长度 */
    const u_char *prefix = &p->u.prefix;
     ...
     */
    while (node && node->p.prefixlen <= prefixlen &&
           prefix_match (&node->p, p)) /* 根据路由前缀 */
    {
        if (node->p.prefixlen == prefixlen) /* 路由已经存在 */
    ...
    else
    {
        ...
        route_common (&node->p, p, &new->p); /* 产生公共路由前缀,也就是说new的前缀要小于等于node */
        new->p.family = p->family;
        new->table = table;
        set_link (new, node);

        if (match)
            set_link (match, new); /* match的子节点是new,即match的前缀短于new */
        else
            table->top = new;

        if (new->p.prefixlen != p->prefixlen) /* new的前缀长度小于p */
        {
            match = new;
            new = route_node_set (table, p);
            set_link (match, new);
