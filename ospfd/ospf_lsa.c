...
ospf_lsa_refresh_delay (struct ospf_lsa *lsa)
{
    ...
    delta = tv_sub (now, lsa->tv_orig); /* 获取差值 */
...
lsa_link_ptop_set (struct stream *s, struct ospf_interface *oi)
{
    ...
    if ((nbr = ospf_nbr_lookup_ptop (oi)))
        if (nbr->state == NSM_Full)
        {
            ...
            /* link_id为邻居的router-id,link_data为自己的ip地址,拓扑信息 */
...
lsa_link_broadcast_set (struct stream *s, struct ospf_interface *oi)
{
    ...
    if (oi->state == ISM_Waiting)
    {
        ...
        /* link_id为网络号,link_data为mask,这里描述了一条路由信息 */
        ...
        ospf_nbr_count (oi, NSM_Full) > 0)
    {
        ...
        /* link_id为DR的接口地址, link_data为接口自己的ip,拓扑信息*/
    ...
    for (ALL_LIST_ELEMENTS_RO (area->oiflist, node, oi))
    {
        ...
        if (if_is_operative (ifp))
        {
            if (oi->state != ISM_Down)
            {
                ...
                /* Describe each link. */
                switch (oi->type) /* 描述每一条链路 */
                {
                    case OSPF_IFTYPE_POINTOPOINT: /* 点到点 -- 串口 */
                        links += lsa_link_ptop_set (s, oi);
                        break;
                    case OSPF_IFTYPE_BROADCAST: /* 广播 -- 以太网 */
                    ...
                    case OSPF_IFTYPE_POINTOMULTIPOINT:
                        ...
                        break;
                    case OSPF_IFTYPE_VIRTUALLINK: /* 虚链路 */
                        links += lsa_link_virtuallink_set (s, oi);
                        break;
                    case OSPF_IFTYPE_LOOPBACK: /* 回环口 -- 路由信息 */
...
ospf_router_lsa_originate (struct ospf_area *area)
{
    ...
    /* 自己产生的lsa都会打上OSPF_LSA_SELF标记,最终会安装到area->router_lsa_self上去 */
    /* Sanity check.
     * 通告路由器
     */
...
ospf_network_lsa_body_set (struct stream *s, struct ospf_interface *oi)
{
    ...
    masklen2ip (oi->address->prefixlen, &mask); /* 掩码 */
                           ...
                           u_int32_t metric)
{
    ...
    /* Put Network Mask. */
    stream_put_ipv4 (s, mask.s_addr); /* 设置掩码 */

    /* Set # TOS. */
    stream_putc (s, (u_char) 0); /* 设置tos */

    /* Set metric. */
    stream_put_ospf_metric (s, metric); /* 设置度量值 */
}

/*
 * 创建一条summary-lsa
 * 一条summary-lsa只包含一个网络信息
 * p 表示掩码(事实上也只用到了掩码值), metric表示度量值
 * id表示网络号
 */
static struct ospf_lsa *
                         ...
                         int rt_recalc)
{
    ...
    */
    ...
    if (IS_LSA_SELF (new)) /* 判断lsa是否为自己产生的 */
    {
        ...
        ospf_lsa_unlock (&area->router_lsa_self);
        area->router_lsa_self = ospf_lsa_lock (new); /* 被引用了,引用计数就会+1 */
		/* 刷新计时器 */
        ospf_refresher_register_lsa (ospf, new);
    }
    if (rt_recalc)
                      ...
                      struct ospf_lsdb *lsdb, struct ospf_lsa *lsa)
{
    ...
    {
        case OSPF_AS_EXTERNAL_LSA: /* 导入的lsa */
        ...
        case OSPF_OPAQUE_AS_LSA:
            ospf_ls_retransmit_delete_nbr_as (ospf, old);
            break;
        case OSPF_AS_NSSA_LSA: /* nssa中的lsa */
			/* 从重传列表中移除 */
                  ...
                  struct ospf_lsa *lsa)
{
    ...
    /*  RFC 2328 13.2.  Installing LSAs in the database
          ...
          将新的lsa保存到数据库汇总，不管这个lsa是通过洪泛，还是自身产生的
          可能会导致ospf路由表结构重新计算,新lsa会和旧的lsa进行比较,如果一致
          不会进行安装操作,在比较的时候，如果下面的值不一致的话，会认为发生了变化
...
ospf_lsa_maxage (struct ospf *ospf, struct ospf_lsa *lsa)
{
                              ...
                              (struct prefix *)&lsa_prefix)) != NULL)
    {
        ...
        else
        {
            ...
            SET_FLAG(lsa->flags, OSPF_LSA_IN_MAXAGE); /* lsa打上标记,可以移除掉了 */
        }
    }
    else
    {
