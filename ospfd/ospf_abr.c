...
ospf_area_range_new (struct prefix_ipv4 *p)
{
    ...
    range->addr = p->prefix; /* 地址 */
    range->masklen = p->prefixlen; /* 掩码 */
                           ...
                           struct prefix_ipv4 *p, struct ospf_route *or)
{
    ...
    /* or_area指代的是路由所属的区域,也就是通告此路由的区域 */
    or_area = ospf_area_lookup_by_area_id (ospf, or->u.std.area_id);
    assert (or_area);

    for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area)) /* 遍历每一个区域 */
    {
        if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("ospf_abr_announce_network(): looking at area %s",
                        inet_ntoa (area->area_id));

        if (IPV4_ADDR_SAME (&or->u.std.area_id, &area->area_id)) /* 不处理属于这个区域的路由 */
            continue;
        /* 判断路由的下一跳是否属于此区域 */
        ...
        if (or->path_type == OSPF_PATH_INTRA_AREA)
        {
            ...
            if ((range = ospf_area_range_match (or_area, p))
                ...
                /* 一旦此条路由匹配路由汇聚规则，那么不通告过去,后面会进行统一的通告 */
...
ospf_abr_announce_aggregates (struct ospf *ospf)
{
    ...
    for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
    {
        ...
        for (rn = route_top (area->ranges); rn; rn = route_next (rn)) /* 遍历路由汇聚 */
            if ((range =  rn->info))
            {
                if (!CHECK_FLAG (range->flags, OSPF_AREA_RANGE_ADVERTISE)) /* 如果不需要通告就不通告 */
                {
                    if (IS_DEBUG_OSPF_EVENT)
