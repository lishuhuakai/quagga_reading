            ...
            struct stream * s, struct ospf_interface *oi, int size)
{
    ...
    /* get neighbor prefix. */
    p.family = AF_INET;
    p.prefixlen = ip_masklen (hello->network_mask); /* 掩码 */
    p.u.prefix4 = iph->ip_src; /* 源ip */

    /* Compare network mask. */
    /* Checking is ignored for Point-to-Point and Virtual link. */
    if (oi->type != OSPF_IFTYPE_POINTOPOINT
        && oi->type != OSPF_IFTYPE_VIRTUALLINK)
        if (oi->address->prefixlen != p.prefixlen) /* 掩码不一致,要丢弃 */
        {
...
#define REJECT_IF_TBIT_ON   1 /* XXX */
                                ...
                                size - OSPF_HELLO_MIN_SIZE))
    {
        ...
        nbr->options |= hello->options; /* 记录下邻居的可选字段信息 */
    }
    else
    {
        /* 进入初始状态 */
     ...
     * 如果邻居声明自己为DR,而且BDR不存在
     */
    if (IPV4_ADDR_SAME (&nbr->address.u.prefix4, &hello->d_router))
        if (hello->bd_router.s_addr == 0 && oi->state == ISM_Waiting)
            OSPF_ISM_EVENT_SCHEDULE (oi, ISM_BackupSeen); /* 进入备份状态 */
    ...
    if ((IPV4_ADDR_SAME (&nbr->address.u.prefix4, &hello->d_router) &&
        ...
        OSPF_ISM_EVENT_SCHEDULE (oi, ISM_NeighborChange); /* 邻居变化 */
                   ...
                   u_int16_t size)
{
    ...
    if (IS_SET_DD_MS (nbr->dd_flags))
    {
        ...
        if (!IS_SET_DD_M (dd->flags) && !IS_SET_DD_M (nbr->dd_flags))
            ...
            /*
             * 自己是从路由器,那就发送dd包给主路由器,应该是发送确认信息
             * dd包中也携带了自己的lsa摘要信息
             */
            ospf_db_desc_send (nbr);
    }
    /* Slave */
    else /* 邻居是从路由器,自己为主路由器 */
    {
              ...
              struct stream *s, struct ospf_interface *oi, u_int16_t size)
{
    ...
    switch (nbr->state) /* 根据邻居的状态来进行相应的处理 */
    {
        case NSM_Down:
        case NSM_Attempt:
        case NSM_TwoWay:
        ...
        case NSM_ExStart: /* 信息交换初始状态 */
            ...
            /* Ack from the Slave */
            /* 邻居是从路由器,而且不是第一个DD包,说明这是一个从路由器发送的确认包 */
        ...
        case NSM_Exchange: /* 信息交换状态 */
            if (ospf_db_desc_is_dup (dd, nbr)) /* DD包重复 */
            {
                if (IS_SET_DD_MS (nbr->dd_flags))   /* 主路由器直接丢弃掉这个包 */
             ...
             struct stream *s, struct ospf_interface *oi, u_int16_t size)
{
    ...
    while (size >= OSPF_LSA_KEY_SIZE)
    {
        ...
        ls_id.s_addr = stream_get_ipv4 (s); /* 链路状态id */
        adv_router.s_addr = stream_get_ipv4 (s); /* 通告路由器 */
        ...
        listnode_add (ls_upd, find); /* 将节点添加到更新列表之中 */
                      ...
                      struct ospf_interface *oi, size_t size)
{
         ...
         size -= length, stream_forward_getp (s, length), count--)
    {
        ...
        memcpy (lsa->data, lsah, length); /* 这里直接将数据拷贝了过去 */
    ...
    for (ALL_LIST_ELEMENTS (lsas, node, nnode, lsa))
    {
        ...
        if (IS_DEBUG_OSPF_NSSA)
        {
            ...
            zlog_debug("LSA Type-%d from %s, ID: %s, ADV: %s",
                       ...
                       inet_ntop (AF_INET, &lsa->data->id, /* 链路状态id */
                                  buf2, INET_ADDRSTRLEN),
                       inet_ntop (AF_INET, &lsa->data->adv_router, /* 通告路由器 */
        ...
        if (lsa->data->type == OSPF_ROUTER_LSA) /* 路由器lsa */
            ...
            ospf_check_nbr_status(oi->ospf))
        {
            ...
            ospf_ls_ack_send (nbr, lsa); /* 发送更新确认包 */
 ...
 * 处理OSPF链路状态通告确认数据包
 * 也就是说,一旦接收到了确认信息,那么要将一些lsa从重传链表中删除
 */
static void
...
ospf_read (struct thread *thread)
{
    ...
    else if (oi->state == ISM_Down) /* 端口没有起来 */
    {
     ...
     * 非DR不接收源自 224.0.0.6的数据包,这种包只有DR和BDR才接收
     */
    ...
    switch (ospfh->type)
    {
        ...
        case OSPF_MSG_LS_REQ: /* 处理lsa请求包 */
            ...
            break;
        case OSPF_MSG_LS_UPD: /* 处理链路状态更新数据包 */
...
ospf_make_header (int type, struct ospf_interface *oi, struct stream *s)
{
    ...
    ospfh->version = (u_char) OSPF_VERSION; /* 版本号 */
    ospfh->type = (u_char) type; /* 数据包类型, 1表示hello包 */

    ospfh->router_id = oi->ospf->router_id; /* 始发路由器id */

    ospfh->checksum = 0;
    ospfh->area_id = oi->area->area_id; /* 始发路由器所在区域 */
...
ospf_make_hello (struct ospf_interface *oi, struct stream *s)
{
    ...
    stream_put_ipv4 (s, mask.s_addr); /* 设置网络掩码 */

    /* Set Hello Interval. 设置hello时间间隔 */
    if (OSPF_IF_PARAM (oi, fast_hello) == 0)
    ...
    /* Add neighbor seen. 列举所有邻居的router id*/
...
ospf_hello_send (struct ospf_interface *oi)
{
    ...
    if (oi->type == OSPF_IFTYPE_NBMA)
    {
        ...
        for (rn = route_top (oi->nbrs); rn; rn = route_next (rn))
            if ((nbr = rn->info))
                if (nbr != oi->nbr_self)
                    if (nbr->state != NSM_Down) /* 邻居没有down掉 */
                    {
...
ospf_ls_ack_send_delayed (struct ospf_interface *oi)
{
    ...
    while (listcount (oi->ls_ack))
        ospf_ls_ack_send_list (oi, oi->ls_ack, dst); /* 发送确认包 */
}
