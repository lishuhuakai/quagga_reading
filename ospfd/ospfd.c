...
add_ospf_interface (struct connected *co, struct ospf_area *area)
{
    ...
    oi->connected = co; /* 记录下这个接口对应的网段信息 */
                            ...
                            struct ospf_area *given_area)
{
    ...
    if (memcmp (ifp->name, "VLINK", 5) == 0) /* 不考虑虚链路 */
        return;
     ...
     * 如果一个接口的前缀和某个特定的前缀不匹配,就创建一个socket,并加入多播组
     */
    for (ALL_LIST_ELEMENTS_RO (ifp->connected, cnode, co)) /* 遍历接口的网络地址 */
                           ...
                           struct ospf_area *area, const char *list_name)
{
    ...
    list = access_list_lookup (AFI_IP, list_name); /* 先找到list */
