       ...
       "Network prefix to be announced instead of range\n")
{
    ...
    VTY_GET_OSPF_AREA_ID (area_id, format, argv[0]); /* area id */
    VTY_GET_IPV4_PREFIX ("area range", p, argv[1]); /* 网络 */
...
ospf_find_vl_data (struct ospf *ospf, struct ospf_vl_config_data *vl_config)
{
    ...
    if (area_id.s_addr == OSPF_AREA_BACKBONE) /* 虚链路不能穿越骨干区域 */
    {
        vty_out (vty,
