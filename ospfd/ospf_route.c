                       ...
                       struct ospf_area *area)
{
    ...
    if (!OSPF_IS_AREA_BACKBONE(area)) /* 非骨干区域 */
...
{
    /* 对于类型为3的lsa,link_id为网络号,link_data为掩码 */
    u_int32_t cost;
    struct route_node *rn;
    struct ospf_route *or;
    struct prefix_ipv4 p;
    ...
    if (rn->info)
    {
        ...
        if (cost < cur_or->cost) /* 现在的路由比原来的路由开销更小,因此删除原来的路由 */
        {
            if (IS_DEBUG_OSPF_EVENT)
        ...
        if ((or = rn->info) != NULL)
        {
            if (or->type == OSPF_DESTINATION_NETWORK) /* 达到一个网络 */
            {
                ...
                for (ALL_LIST_ELEMENTS_RO (or->paths, pnode, path))
                    zlog_debug ("  -> %s", inet_ntoa (path->nexthop)); /* 下一跳地址 */
            }
            else
