...
rib_nexthop_ifindex_add (struct rib *rib, ifindex_t ifindex)
{
    ...
    nexthop = nexthop_new ();
    nexthop->type = NEXTHOP_TYPE_IFINDEX; /* 下一跳是一个网络设备 */
...
rib_nexthop_ipv4_add (struct rib *rib, struct in_addr *ipv4, struct in_addr *src)
{
    ...
    nexthop = nexthop_new ();
    nexthop->type = NEXTHOP_TYPE_IPV4; /* 网关是一个ipv4地址 */
                      ...
                      struct nexthop *nexthop, int set)
{
    ...
    switch (nexthop->type)
    {
        case NEXTHOP_TYPE_IFINDEX: /* 下一跳是一个接口 */
        ...
        case NEXTHOP_TYPE_IFNAME: /* 接口名称 */
            ...
            else
            {
                if (set)
                    nexthop->ifindex = 0;
                UNSET_FLAG (nexthop->flags, NEXTHOP_FLAG_ACTIVE); /* 下一跳不可达 */
            }
            break;
 ...
 * RIB的更新是通过一系列指向route_nodes的队列来完成的
 ...
 * RIBs通过rib_addnode或者rib_delnode来提交
 *
...
static_install_route (afi_t afi, safi_t safi, struct prefix *p, struct static_route *si)
{
    ...
    /* Lookup table.  */
    table = zebra_vrf_table (afi, safi, si->vrf_id); /* 查找路由表 */
    if (! table)
        return;

    /* Lookup existing route */
    rn = route_node_get (table, p);
    RNODE_FOREACH_RIB (rn, rib) /* 遍历rib链表 */
    {
        if (CHECK_FLAG (rib->status, RIB_ENTRY_REMOVED))
            continue;
        /* 静态路由并且distance一致 */
                      ...
                      u_char distance, vrf_id_t vrf_id)
{
    ...
    struct route_table *stable = zvrf->stable[AFI_IP][safi]; /* ipv4静态路由表 */
    ...
    if (gate)
        si->addr.ipv4 = *gate; /* 网关地址 */
    if (ifname)
        si->ifname = XSTRDUP (MTYPE_TMP, ifname); /* 端口名称 */
    ...
    /* Install into rib. */
    static_install_route (AFI_IP, safi, p, si); /* ipv4路由信息 */
...
zebra_vrf_table_create (struct zebra_vrf *zvrf, afi_t afi, safi_t safi)
{
    ...
    zvrf->table[afi][safi] = table; /* 记录下表信息 */
